...
export function LineItemsTable({ currentCase }: LineItemsTableProps) {
  ...
  const getViolationBadge = (type: SampleLineItem["violationType"]) => {
    switch (type) {
      case "UPCODING":
        return (
          <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-mono font-semibold bg-red-500/15 border border-red-500/35 text-red-400 shadow-[0_0_8px_rgba(239,68,68,0.15)]">
            UPCODED · LVL 5
          </span>
        );
      case "UNBUNDLING":
        return (
          <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-mono font-semibold bg-amber-500/15 border border-amber-500/35 text-amber-300 shadow-[0_0_8px_rgba(245,158,11,0.15)]">
            UNBUNDLED
          </span>
        );
      case "PRICE_GOUGE_OVER_CHARGEMASTER":
        return (
          <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-mono font-semibold bg-rose-500/15 border border-rose-500/35 text-rose-300 shadow-[0_0_8px_rgba(244,63,94,0.15)]">
            8.3× MARKUP
          </span>
        );
      case "COMPLIANT":
  ...
  const getRowBgClass = (item: SampleLineItem, isExcluded: boolean, isExpanded: boolean) => {
    if (isExcluded) return "opacity-40 bg-black/30";
    if (isExpanded) return "bg-white/[0.05]";
    switch (item.violationType) {
      ...
      case "PRICE_GOUGE_OVER_CHARGEMASTER":
        return "bg-rose-500/[0.03] hover:bg-rose-500/[0.07]";
      case "COMPLIANT":
      default:
  ...
  return (
    <div className="bg-[#080c13] border border-white/[0.08] rounded-xl overflow-hidden shadow-sm">
      {/* Table Toolbar Strip */}
      <div className="px-4 py-3 border-b border-white/[0.06] bg-[#0b1018] flex flex-col md:flex-row md:items-center justify-between gap-3">
        {/* Left: Filter Tabs */}
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`px-2.5 py-1 text-xs font-mono font-medium rounded-lg border transition-all flex items-center gap-1.5 cursor-pointer ${
          ...
          <button
            ...
            className="px-2.5 py-1 text-xs font-mono rounded-lg bg-white/[0.03] hover:bg-white/[0.06] border border-white/[0.08] text-slate-400 hover:text-slate-200 transition-colors flex items-center gap-1 cursor-pointer"
          >
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-white/[0.08] bg-black/40 text-[10px] uppercase tracking-wider font-mono text-slate-400">
              ...
              <th className="py-2.5 px-4 text-right text-emerald-400 font-semibold">Legal Settlement</th>
            ...
            {displayedItems.map((item) => {
              ...
              return (
                <React.Fragment key={item.id}>
                  ...
                  >
                    ...
                    >
                      <input
                        ...
                        className="w-4 h-4 rounded border-white/[0.2] bg-black text-emerald-500 focus:ring-0 cursor-pointer accent-emerald-500 transition-transform active:scale-90"
                    ...
                    <td className="py-3 px-4">
                      <div className="font-mono font-bold text-slate-100 text-xs tracking-wide">
                        {item.cptCode}
                      </div>
                    ...
                    <td className="py-3 px-4 text-right tabular-nums">
                      ...
                      {item.isDisputed && overchargeAmount > 0 && (
                        <div className="text-[10px] text-emerald-500/90 font-mono font-medium">
                  ...
                  {isExpanded && (
                    <tr className="bg-[#05080d] border-b border-white/[0.06]">
                      <td colSpan={8} className="p-4 sm:px-6 space-y-3">
                        ...
                        <div className="p-3 rounded-lg bg-black/40 border border-white/[0.06] space-y-2">
                          <div className="flex items-center justify-between text-[11px] font-mono">
                            <span className="text-slate-400">Rate Discrepancy Breakdown for CPT {item.cptCode}</span>
                          ...
                          <div className="flex justify-between text-[10px] font-mono text-slate-500 pt-0.5">
                            <span>■ Cash Baseline (${item.proposedAmount})</span>
                            <span className="text-rose-400">■ Strike Margin: -${overchargeAmount}</span>
                          </div>
                        </div>
                          ...
                          <div className="p-3.5 rounded-lg bg-white/[0.02] border border-white/[0.06] space-y-2">
                            ...
                            <div className="space-y-1 text-[11px] text-slate-300">
                              <div className="flex justify-between">
                                ...
                                <span className="text-slate-200 font-medium">${item.billedAmount.toLocaleString("en-US", { minimumFractionDigits: 2 })}</span>
                              </div>
                              <div className="flex justify-between">
                              ...
                              <div className="flex justify-between">
                                ...
                                <span className="text-emerald-400 font-medium">${item.cmsBenchmarkRate.toLocaleString("en-US", { minimumFractionDigits: 2 })}</span>
                              </div>
                            </div>
                          </div>
                        </div>
      ...
      {/* Footer explanation */}
      <div className="px-4 py-2.5 bg-[#0b1018] border-t border-white/[0.06] flex flex-col sm:flex-row items-center justify-between text-[11px] font-mono text-slate-500 gap-2">
        <div className="flex items-center gap-2">
